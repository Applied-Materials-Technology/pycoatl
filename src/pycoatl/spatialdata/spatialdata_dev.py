...
class SpatialData():
    ...
    def get_mesh_component(self,data_field_name: str,component: Sequence, time_step: int) -> pv.UnstructuredGrid:
        ...
        Args:
            data_field_name (str): _description_
            component (Sequence): _description_

        Returns:
            pv.UnstructuredGrid: _description_
        """
        output_mesh = pv.UnstructuredGrid()
        output_mesh.copy_from(self.mesh_data)
        output_mesh[data_field_name] = self.data_fields[data_field_name.get_component(component,time_step)]
