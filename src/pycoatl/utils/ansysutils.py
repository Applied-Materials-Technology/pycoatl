#
# Code for reading ansys binaries
#
...
def read_ansys_binary(filename,datfile,nodfile):
    ...
    Getting the reaction forces is particularly difficult as it lacks an internal pointer (have figured in out)
...
def read_ansys_binary_piecewise(filename,nlist_file):
    """ Gets all the important results for each time step from the .rst ANSYS results file.
    .rst are binary file types and require the PyAnsys package to read. 
    Getting the reaction forces is particularly difficult as it lacks an internal pointer (have figured in out)
    ...
    def getFD(result,step):
        ...
        #rforces, nnum, dof = result.nodal_reaction_forces(step,False) # Old version of Pyansys
        rforces, nnum, dof = result.nodal_reaction_forces(step) #MAPDL reader version
        ...
        FEAdisp = np.mean(bcd[n][:p]) # Note boundary displacement on each node should be identical (as they're defined that way)
        #FEAload = np.abs(np.sum(rforces[n][:p]))
        FEAload = np.abs(np.sum(rforces[n][np.where(bcd[bdof==2]==0)]))
    ...
    def read_nlist(file_name):
        """
        Method to read the NLIST file produced by APDL

        Parameters
        ----------
        file_name : string
            Path to the NLIST file
        ...
        """
        data = pd.read_fwf(file_name,widths=[10,14,14,14,10],skiprows=12)
        df = data[data['THXY']=='0.00']
        
        node_num = df['NODE'].to_numpy(dtype= np.int32)
        x= df['X'].to_numpy(dtype= np.float32)
        y= df['Y'].to_numpy(dtype= np.float32)
        z= df['Z'].to_numpy(dtype= np.float32)
        
        return node_num, x, y, z
    
    
    # Main method, to loop through result file and extract only the appropriate steps' data
   
    # Get coordinate information from the .dat file and the specified surface nodes 
    node_nums,nnx,nny, nnz = read_nlist(nlist_file)
    ...
    FEA_disp = np.empty(len(lstabc))
    FEA_load = np.empty(len(lstabc))
    FEA_equiv_p_strain = np.empty((len(lstabc),len(node_nums)))
    FEA_equiv_stress = np.empty((len(lstabc),len(node_nums)))
    FEA_surf_x_disp = np.empty((len(lstabc),len(node_nums)))
    FEA_surf_y_disp = np.empty((len(lstabc),len(node_nums)))
    FEA_surf_z_disp = np.empty((len(lstabc),len(node_nums)))
    #FEA_surf_strainE = np.empty((len(lstabc),len(node_nums)))
    ...
    for counter,resultStep in enumerate(lstabc):
        FEA_disp[counter],FEA_load[counter] = getFD(result,resultStep)
        FEA_equiv_stress[counter,:],FEA_equiv_p_strain[counter,:],FEA_surf_x_strain[counter,:],FEA_surf_y_strain[counter,:],FEA_surf_xy_strain[counter,:] = get_surface_ss(result,resultStep,node_nums)
        FEA_surf_x_disp[counter,:], FEA_surf_y_disp[counter,:], FEA_surf_z_disp[counter,:] = get_surface_displacement(result,resultStep,node_nums)
        #FEA_surf_strainE[counter,:] = get_elastic_strain(result,resultStep,node_nums)
        
        # old # FEA_disp,FEA_load, FEA_surf_strain, FEA_surf_stress, FEA_surf_x_strain, FEA_surf_y_strain,FEA_surf_xy_strain

    return FEA_x, FEA_y, FEA_z, FEA_surf_x_disp,FEA_surf_y_disp, FEA_surf_z_disp, FEA_surf_x_strain,FEA_surf_y_strain,FEA_surf_xy_strain, FEA_load, FEA_equiv_p_strain, FEA_equiv_stress 
 
 
def read_ansys_binary_piecewise_full(filename,nlist_file):
    """ Gets all the important results for each time step from the .rst ANSYS results file.
    .rst are binary file types and require the PyAnsys package to read. 
    Getting the reaction forces is particularly difficult as it lacks an internal pointer (have figured in out)
    ...
    def getFD(result,step):
        ...
        rforces, nnum, dof = result.nodal_reaction_forces(step)
        #dof_ref = result.result_dof(step) 
        n = dof ==2 
    ...
    # Preallocate arrays
    FEA_disp = np.empty(result.nsets)
    FEA_load = np.empty(result.nsets)
    FEA_equiv_p_strain = np.empty((result.nsets,len(node_nums)))
    FEA_equiv_stress = np.empty((result.nsets,len(node_nums)))
    FEA_surf_x_disp = np.empty((result.nsets,len(node_nums)))
    FEA_surf_y_disp = np.empty((result.nsets,len(node_nums)))
    FEA_surf_z_disp = np.empty((result.nsets,len(node_nums)))
    #FEA_surf_strainE = np.empty((result.nsets,len(node_nums)))
    FEA_surf_x_strain = np.empty((result.nsets,len(node_nums)))
    FEA_surf_y_strain = np.empty((result.nsets,len(node_nums)))
    FEA_surf_xy_strain = np.empty((result.nsets,len(node_nums)))
    
    for counter in range(result.nsets):
        resultStep=counter
        FEA_disp[counter],FEA_load[counter] = getFD(result,resultStep)
        FEA_equiv_stress[counter,:],FEA_equiv_p_strain[counter,:],FEA_surf_x_strain[counter,:],FEA_surf_y_strain[counter,:],FEA_surf_xy_strain[counter,:] = get_surface_ss(result,resultStep,node_nums)
        FEA_surf_x_disp[counter,:], FEA_surf_y_disp[counter,:], FEA_surf_z_disp[counter,:] = get_surface_displacement(result,resultStep,node_nums)
